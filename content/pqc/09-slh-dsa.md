...
description: "Build intuition from Lamport signatures and Merkle paths to WOTS+, FORS, stateless hypertrees, exact SLH-DSA sizes, and limited-use draft parameters."
slug: pqc-09-slh-dsa
...
This part starts with one-time signatures and builds toward the stateless hash-based signature algorithm in [FIPS 205](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf). You need the hash-function and signature concepts from Part 2, but no lattice algebra. By the end, you should understand the role of hash chains, authentication paths, FORS, and multiple tree layers, and be able to select a benchmark that reflects your actual workload.

FIPS 205 was finalized on August 13, 2024. Its twelve parameter sets are standardized. As checked on September 11, 2026, [SP 800-230](https://csrc.nist.gov/pubs/sp/800/230/ipd), proposing additional limited-signature-use sets, remains an initial public draft published April 13, 2026. Its closed comment period does not make it final. Keep those two status claims separate.

SLH-DSA descends from SPHINCS+, whose [research paper](https://eprint.iacr.org/2019/1086) explains the design's development. Historical SPHINCS+ parameter names and implementations are useful research context; production interoperability must name the final SLH-DSA parameter set and encoding. “Hash-based” describes a family, not a universal wire format.
...
XMSS, described in [RFC 8391](https://www.rfc-editor.org/rfc/rfc8391.html), combines one-time signatures with Merkle trees and carefully managed state. The signer tracks which leaf index has already been used. NIST's [SP 800-208](https://csrc.nist.gov/pubs/sp/800/208/final) profiles stateful hash-based signatures for approved use under specified conditions.
...
The [OpenSSL SLH-DSA manual](https://docs.openssl.org/3.5/man7/EVP_SIGNATURE-SLH-DSA/) documents the native interface. The script below was executed with OpenSSL 3.5.8 on macOS ARM64. It verifies both SHA2 category-1 variants, checks exact signature sizes, and rejects a changed message. It is a functional experiment, not a timing benchmark.
